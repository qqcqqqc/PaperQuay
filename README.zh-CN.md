...
- **easyScholar 深度集成：** 支持从 `easyscholar.cc` 自动获取期刊影响因子 (IF)、中科院分区、CCF 等级及各大高校期刊资助分级。数据在文库列表和详情页实时展示并持久化存储。
- **全文翻译断点续传：** 优化全文翻译逻辑，支持准确的进度恢复显示；后端改为并行翻译模式，显著提升速度，并确保部分失败时不丢失已完成的译文。
- **深色模式体验优化：** 彻底修复了"双栏对照"、图片预览、表格容器和公式块在深色模式下的白底/白字不可见问题。
- **概览生成逻辑优化：** 针对已存在概览的文献，在重新生成前增加确认提示，防止 Token 浪费和误操作。
- **Zotero 补充材料作为附件导入：** Zotero 导入时自动将主文献与补充 PDF 分组，补充材料作为附件挂载到主文献下，不再以重复条目形式导入，支持批量导入与附件自动关联。
- **PDF 附件全生命周期管理：** 支持在文献库中通过右键菜单或详情面板的 `+` 按钮添加 PDF 附件；支持删除附件（主题风格确认框）；附件列表点击即可打开对应 PDF。
- **按附件独立执行 MinerU 解析与全文翻译：** MinerU 解析和全文翻译现在支持通过下拉菜单选择具体要处理的 PDF 附件（不仅限于主 PDF），可单独对补充材料进行结构化提取和翻译。
- **刷新自动清理缺失 PDF 文献：** 文库刷新时会自动检测并删除 PDF 文件已不存在的文献记录，保持文库干净整洁。

**划词弹窗重构 & 批注橡皮擦工具：**

- **划词弹窗拖拽与缩放：** 弹窗新增拖拽手柄（可任意拖动位置）和缩放手柄（可调整大小），设有最小宽高限制。
- **快捷高亮色块：** 弹窗内直接提供 6 色预设高亮（黄、绿、青、粉、红、紫），无需切换工具栏即可快速标记。
- **批注橡皮擦工具：** 工具栏新增橡皮擦按钮，点击后进入擦除模式，直接点击页面上的手写或文本批注即可删除。同时移除了旧的"Delete Selected"按钮，交互更直观。
- **弹窗布局溢出修复：** 缩到很小时，译文文本框和控件不再跑到弹窗圆角之外。
- **首次拖拽瞬移修复：** 修复弹窗刚打开时第一次拖拽会乱飘飞出屏幕的 bug。
